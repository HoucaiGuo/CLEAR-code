...
def fill_single_image_kd_tree(ref_images,
    ...
    for class_idx in range(class_num):
        ...
        temporal_weights = all_coefficients[class_idx - skiped_class_num, :, :]  # shape = (band_num, ref_num)
        ...
        for cloudy_idx in trange(cloudy_num, position=0, leave=True):
            ...
            similar_spec_temp_diffs_norm = ((similar_spec_temp_diffs - similar_spec_temp_diffs.min()) /
                                            (similar_spec_temp_diffs.max() - similar_spec_temp_diffs.min() + 0.00001) + 1)
            ...
            similar_weights = ((1 / (similar_distances_norm * similar_spec_temp_diffs_norm)) /
                               np.sum((1 / (similar_distances_norm * similar_spec_temp_diffs_norm))))
