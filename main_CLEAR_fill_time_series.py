...
if __name__ == "__main__":
    ...
    for idx in range(candidate_images.shape[3]):
        ...
        if candidate_cloud_cover > 0:
            ...
            filled_candidate_image = fill_single_image_fast(other_candidate_images,
                                                            ...
                                                            similar_num)[1]
            filled_candidate_image[filled_candidate_image < min_value] = min_value
            filled_candidate_image[filled_candidate_image > max_value] = max_value
    ...
    filled_images_list = [filled_images[:, :, :, i] for i in range(filled_images.shape[3])]
    filled_images = np.concatenate(filled_images_list, axis=2)
    filled_images[filled_images < min_value] = min_value
    filled_images[filled_images > max_value] = max_value
    filled_images = filled_images.astype(np.uint16)
